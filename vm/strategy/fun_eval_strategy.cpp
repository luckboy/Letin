...
namespace letin
{
  namespace vm
  {
    namespace impl
    {
      ...
      { 
        _M_fun_pairs = unique_ptr<FunctionPair []>(new FunctionPair[fun_count]);
        size_t unmemoized_fun_count = 0;
        ...
        for(size_t i = 0; i < fun_count; i++) {
          ...
          if((eval_strategy & EVAL_STRATEGY_MEMO) == 0) {
            _M_fun_pairs.get()[i].eval_strategy_fun_index = unmemoized_fun_count;
            unmemoized_fun_count++;
          } else {
            _M_fun_pairs.get()[i].eval_strategy_fun_index = memoized_fun_count;
            memoized_fun_count++;
          }
          _M_fun_pairs.get()[i].eval_strategy = eval_strategy;
        }
        _M_eager_eval_strategy.set_fun_infos_and_fun_count(nullptr, unmemoized_fun_count);
        _M_lazy_eval_strategy.set_fun_infos_and_fun_count(nullptr, unmemoized_fun_count);
